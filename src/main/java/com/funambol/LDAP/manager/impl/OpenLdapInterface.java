...
/**
 * OpenLdap Interface - uses cache
 * @author rpolli@babel.it
 *
 */
public class OpenLdapInterface extends AbstractLDAPManager {

	public static String LDAPID =  "EntryUUID";
	

	
	// bean constructor
	public OpenLdapInterface(){
	}

	// standard constructor for accessing ldap
	public OpenLdapInterface(String providerUrl, String baseDn, String binddn,
	...
	public String  addNewEntry(SyncItem si) {
		...
		if (logger.isInfoEnabled()) {
			logger.info("OpenLdapInterface.addNewEntry(). " );
		}
		if (logger.isDebugEnabled()) {
			logger.debug(String.format("cdao: %s, cache: %s, ", cdao, cache));
		}
		String newEntryNsUniqueId = null;
		Attributes attrs = cdao.syncItemToLdapAttributes(si);

		// add attribute if timestamp is set
		if ((si.getTimestamp() != null) && (getCdao().getTimestampAttribute() != null)) {
